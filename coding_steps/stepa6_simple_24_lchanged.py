# 第a6步：杆长可变
# 总长2.4m
# 16自由度，8根杆子，n_forcrawling=15，杆长l_forcrawling=0.3m
# 24自由度，12根杆子，n_forcrawling=23，杆长l_forcrawling=0.2m
...
from step9_dot_matrix2 import generate_dots_inside

from crawling.crawling_v993_bugfixed import *

# =====================================================================
# n_forcrawling：自由度个数（关节个数），后简称n
# n_forcrawling = 16  # 取偶数，每两个关节后跟一根杆，各关节标号：01，23，45，67，89，1011，1213，1415（奇数关节后跟一根杆）
n_forcrawling = 24  # 取偶数，每两个关节后跟一根杆，各关节标号：01，23，45，67，89，1011，1213，1415，1617，1819，2021，2223

# l_forcrawling：杆长（单位：米），后简称l
# l_forcrawling = 0.3
l_forcrawling = 0.2
...
# ================================================================================================
# obs：共n+3维。3个，表示目标点的位置；n个，表示n_forcrawling个关节的角度
observationSpace = (n_forcrawling + 3,)
# action：共n维，表示n个dh，用来自运动
...
class arm_robot():
    ...
    def use_message_data(self, action):
        ...
        self.Tforworld = forward_kinematics3(self.n, self.theta_ang, l=l_forcrawling)
        ...
        while cal_distance(self.goal_position, self.end[0:3]) > 0.01 and count < reach_limit:
            ...
            # plt.plot(self.jointpose[:, 0], self.jointpose[:, 1], self.jointpose[:, 2], '-o', c='orangered',linewidth=2)
            # plt.plot(self.jointpose[:, 0], self.jointpose[:, 1], self.jointpose[:, 2], '-o', c='cornflowerblue',
            #          linewidth=2)
            # plt.pause(0.0001)
        # useful_plot
        # plt.plot(self.jointpose[:, 0], self.jointpose[:, 1], self.jointpose[:, 2], '-o', c='cornflowerblue',
    ...
    def get_reward(self, action=None):
        ...
        for i in range(len(self.jointpose) - 2):
            ...
            for j in range(len(dots)):
                dot = dots[j]
                if check_point_collision(dot[0], dot[1], dot[2], h, r1, x0bucket, y0bucket, z0bucket):
                    collide = True
                    count += 1
                    # print('ohno!\n')
...
if __name__ == '__main__':
    ...
    ax.set_xlim([0, 1.8])
    ax.set_ylim([0.9-0.9, 0.9+0.9])
    ax.set_zlim([0-0.9, 0+0.9])
    ...
    time_consumed = []
    dots_inside = generate_dots_inside(h=h, r=r1, x0bucket=x0bucket, y0bucket=y0bucket, z0bucket=z0bucket)
