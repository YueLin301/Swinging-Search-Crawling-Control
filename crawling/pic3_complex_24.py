...
theta_goal = \
    [-0.17763882,  0.2366849, - 0.46563516, - 0.42248582, - 0.11512268, - 0.93430175,
     1.44701141,  0.79564964, - 0.23162733,  0.77078083, - 1.51537331, - 0.4870423,
     - 1.05659097, - 0.74264436, - 0.72001422, - 0.71870338, - 1.00971649, - 1.3114937,
     1.24617199, - 0.80353062, - 0.3486293,   0.88565183, - 0.19981134, - 1.11176697,]
...
if __name__ == '__main__':
    ...
    ax.plot_surface(xsurface, ysurface2, zsurface2, cmap=plt.get_cmap('gray'), alpha=0.8)
    plt.show()
    ...
    for i in range(len(solq)):
        ...
        ts = np.array(ts)
        # print(ts)
        # plt.plot(ts_goal[i_goal:, 0], ts_goal[i_goal:, 1], ts_goal[i_goal:, 2], '-o', c='orange', linewidth=2)
        plt.plot(ts_goal[:, 0], ts_goal[:, 1], ts_goal[:, 2], '-o', c='orange', linewidth=2)
        plt.plot(ts[:, 0], ts[:, 1], ts[:, 2], '-o', c='cornflowerblue', linewidth=2)
        ax.plot_surface(xsurface, ysurface1, zsurface1, cmap=plt.get_cmap('gray'), alpha=0.3)
        ax.plot_surface(xsurface, ysurface2, zsurface2, cmap=plt.get_cmap('gray'), alpha=0.3)
        plt.pause(0.0001)
